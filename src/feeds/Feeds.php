...
class Feeds extends Component
{
    ...
    public function getFeedItems(string $url, int $limit = null, int $offset = null, string $cacheDuration = null)
    {
        ...
        if (($cached = Yii::$app->getCache()->get($key)) !== false) {
            return $cached;
        }
        ...
        try {
            ...
            Yii::warning('There was a problem parsing the feed: ' . $e->getMessage(), __METHOD__);
        ...
        foreach ($items as $item) {
            ...
            if ($permalink) {
                ...
                if (!$urlModel->validate()) {
                    Yii::info('An item was omitted from the feed (' . $url . ') because its permalink was an invalid URL: ' . $permalink, __METHOD__);
                    continue;
                }
            }
        ...
        Yii::$app->getCache()->set($key, $return, $cacheDuration);
        return $return;
    }
