...
class Uploader extends Component
{
    ...
    public function up(UploadedFile $file)
    {
        ...
        if (!($file->saveAs($this->filePath) && file_exists($this->filePath))) {
            $this->stateInfo = Yii::t('yuncms', 'An error occurred while saving the file.');
            return false;
        } else { //移动成功
            $this->saveModel();
    ...
    public function upBase64()
    {
        ...
        //检查文件大小是否超出限制
        if (!$this->checkSize()) {
            $this->stateInfo = Yii::t('yuncms', 'The file size exceeds the site limit.');
            return false;
        }
    ...
    public function saveRemote()
    {
        ...
        if (strpos($imgUrl, "http") !== 0) {
            $this->stateInfo = Yii::t('yuncms', 'The link is not an http link.');
            return false;
        }
        $http = new Client();
        $response = $http->get($imgUrl)->send();
        if (!$response->isOk) {
            $this->stateInfo = Yii::t('yuncms', 'The link is not available.');
            return false;
        } else {
            ...
            //检查文件类型
            if (!$this->checkType()) {
                $this->stateInfo = Yii::t('yuncms', 'The link contentType is incorrect.');
                return false;
            }
    ...
    /**
     * 保存到模型
     * @return null|\yuncms\db\ActiveRecord
     */
